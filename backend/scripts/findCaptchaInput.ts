...
(async () => {
  try {
    ...
    await page.goto(env.violation.moiUrl, { waitUntil: 'domcontentloaded', timeout: 45000 });
    ...
    const qidFormInfo = await page.evaluate(() => {
      ...
      const img = form.querySelector('img[id*="captcha" i], img[src*="captcha" i]');
    ...
    const allButtons = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('button, input[type="button"], input[type="submit"]')).map(
        (b: any) => ({
          id: b.id || '',
          name: b.name || '',
          text: b.textContent?.trim().slice(0, 50) || '',
