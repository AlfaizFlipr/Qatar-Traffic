...
export const env = {
  ...
  corsOrigins: optional('CORS_ORIGIN', 'http://localhost:5173,https://qatar-traffic.netlify.app')
    .split(',')
    .map((o) => o.trim())
    .filter(Boolean),
  ...
  violation: {
    ...
    moiUrl: optional('MOI_URL', 'https://fees2.moi.gov.qa/moipay/inquiry/violation'),
  ...
  vpn: {
    ...
    bin: optional('OPENVPN_BIN', 'C:\\Program Files\\OpenVPN\\bin\\openvpn.exe'),
    config: optional(
      'OPENVPN_CONFIG',
  ...
  scraperSelectors: {
    ...
    resultsTable: optional('SEL_RESULTS', 'table.table, .results-table, #resultTable, [id*="result" i]'),
    resultRow: optional('SEL_RESULT_ROW', 'table.table tbody tr, table tbody tr, .result-row'),
    noResults: optional('SEL_NO_RESULTS', '.no-results, .no-data, .alert, [class*="empty" i]'),
    captchaError: optional('SEL_CAPTCHA_ERROR', '.captcha-error, .error-message, .alert-danger, [class*="error" i]'),
  },
} as const;
