...
export const captchaService = {
  async start(input: ViolationSearchInput): Promise<StartResponse> {
    ...
    sessionStore.create({ id, mode: 'simulated', identifier, input, expectedCode: challenge.code });
    return { cached: false, captchaRequired: true, sessionId: id, captchaImage: challenge.image, mode: 'simulated' };
  },

  /** Step 2 — verify the typed CAPTCHA, fetch + parse violations, cache, and return. */
  async submit(sessionId: string, captchaCode: string): Promise<ViolationSearchResult> {
    const session = sessionStore.get(sessionId);
    if (!session) throw new AppError('CAPTCHA session expired. Please search again.', 410);
