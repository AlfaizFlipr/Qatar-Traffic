 ...
 */
export async function fetchHttpResult(input: ViolationSearchInput): Promise<ViolationSearchResult> {
  ...
  const { data } = await axios.post(env.violation.apiUrl, input, {
    timeout: 15000,
    headers: env.violation.apiKey ? { Authorization: `Bearer ${env.violation.apiKey}` } : undefined,
  });
