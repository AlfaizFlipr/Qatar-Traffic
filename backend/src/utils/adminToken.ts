...
function sign(data: string): string {
  return crypto.createHmac('sha256', env.admin.secret).update(data).digest('base64url');
}

export function signToken(username: string): string {
  const payload: TokenPayload = { sub: username, exp: Date.now() + env.admin.tokenTtlMs };
