...
export const violationSearchSchema = z
  ...
  .superRefine((data, ctx) => {
    if (data.searchType === 'vehicle' && !data.plateNumber) {
      ctx.addIssue({ code: 'custom', path: ['plateNumber'], message: 'plateNumber is required for vehicle search' });
    }
    if (data.searchType === 'personal' && !data.personalNumber) {
      ctx.addIssue({ code: 'custom', path: ['personalNumber'], message: 'personalNumber is required' });
    }
    if (data.searchType === 'establishment' && !data.establishmentId) {
      ctx.addIssue({ code: 'custom', path: ['establishmentId'], message: 'establishmentId is required' });
    }
  });
