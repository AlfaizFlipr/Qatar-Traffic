...
export function PaymentModal({
  ...
  const form = useForm({
    ...
    validate: {
      ...
      mobile: (v) =>
        /^[+\d][\d\s-]{5,}$/.test(v.trim()) ? null : t.payment.invalidMobile,
      email: (v) =>
        !v || /^\S+@\S+\.\S+$/.test(v) ? null : t.payment.invalidEmail,
    },
  });
    ...
    >
      {done ? (
        <Stack align="center" gap="md" py="md">
          ...
          <Button fullWidth radius="md" onClick={handleClose}>
            {t.common.backHome}
          </Button>
        </Stack>
      ) : (
